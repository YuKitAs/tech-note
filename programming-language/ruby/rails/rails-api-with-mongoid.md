# Rails API with Mongoid

Mongoid is a Ruby ODM (Object-Document-Mapper) framework for MongoDB in Ruby.

1. To setup a rails API with Mongoid we can omit ActiveRecord which is designed for SQL databases:

  ```console
  $ rails new <project-name> --api -T --skip-active-record
  ```

2. Add `gem 'mongoid', '~> 7.0.0'` into `Gemfile` and run `bundle install`.

3. Create a `mongoid.yml` file in `config` with the following content and modify the configurations:
...
* [Mongoid Documents](https://docs.mongodb.com/mongoid/master/tutorials/mongoid-documents/)
